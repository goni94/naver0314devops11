...
SELECT * FROM ENP WHERE JOB=(select job from emp where ename='ALLEN');
...
--문자함수
SELECT CONCAT('HAPPT','DAY') FROM DUAL;
...
select substr('happy day',-5,3) from dual; --뒤에서 5번째 부터 3글자(첫글자 인덱스는1) : y d
select ename,length(ename) 글자수 from emp; --length : 길이,글자수
select replace('Good Day','o','*') from dual; --ㅇ 을 *로 변경해서 출력
